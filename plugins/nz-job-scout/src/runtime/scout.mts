...
function closingHasPassed(value, now) {
  ...
  if (dateOnly(value)) return value < aucklandDateKey(now);
...
export function normaliseUrl(value) {
  ...
  try {
    ...
    for (const key of [...url.searchParams.keys()]) {
      if (/^(ref|source|tracking|trk|eBP|trackingId|refId|seek-token|origin|utm_.*)$/i.test(key)) url.searchParams.delete(key);
    }
...
function jobIdentityKeys(job) {
  ...
  if (employer && title) {
    ...
    if (!job.requisitionId && !applicationUrl) keys.push(`role-loose:${employer}|${title}`);
  }
  return [...new Set(keys)];
}
...
function validateDateEvidence(job, index, errors) {
  ...
  for (const field of ['postedAt', 'closesAt', 'startAt', 'endAt']) {
    asArray(job?.dateEvidence?.[field]).forEach((entry, evidenceIndex) => {
      ...
      if (!sourceTypes.has(entry?.sourceType)) errors.push(`jobs[${index}].dateEvidence.${field}[${evidenceIndex}].sourceType is invalid`);
      if (!confidenceValues.has(entry?.confidence)) errors.push(`jobs[${index}].dateEvidence.${field}[${evidenceIndex}].confidence is invalid`);
      if (entry?.value) {
        try { parseDate(entry.value, `jobs[${index}].dateEvidence.${field}[${evidenceIndex}].value`); }
...
export function deriveSearchCoverage(searchCoverage = {}, leads = []) {
  ...
  const materialFailures = attempts.filter((attempt) =>
    attempt.requiredForCoverage !== false && ['blocked', 'unavailable', 'discovery-only'].includes(attempt.status));
  const successfulFamilyStrategy = (family, strategy) => searched.some((attempt) =>
    normalise(attempt.roleFamily) === normalise(family) && attempt.strategy === strategy);
  const missingBroadDiscoveryFamilies = families.filter((family) =>
    !successfulFamilyStrategy(family, 'broad-discovery'));
  const missingSourceInventoryFamilies = families.filter((family) =>
    !successfulFamilyStrategy(family, 'source-inventory'));
  const unsearchedFamilies = families.filter((family) =>
    missingBroadDiscoveryFamilies.includes(family) && missingSourceInventoryFamilies.includes(family));
  const requiredEmployers = [...new Set(asArray(leads)
    ...
    .filter(Boolean))];
  const unexpandedEmployers = requiredEmployers.filter((employer) => !searched.some((attempt) =>
    attempt.strategy === 'employer-expansion' && normalise(attempt.employer) === normalise(employer)));
...
export function validateSession(session) {
  const errors = [];
  if (!session || typeof session !== 'object' || Array.isArray(session)) return { valid: false, errors: ['session must be a JSON object'] };
  ...
  asArray(session.preferences?.constraints).forEach((constraint, index) => {
    ...
    if (!['hard', 'soft'].includes(constraint?.strength)) errors.push(`preferences.constraints[${index}].strength must be hard or soft`);
    if (!['user-explicit', 'conversation-context', 'resume-inferred', 'skill-default'].includes(constraint?.source)) errors.push(`preferences.constraints[${index}].source is invalid`);
  });

  if (!session.searchCoverage || typeof session.searchCoverage !== 'object') errors.push('searchCoverage is required');
  else {
    if (!Array.isArray(session.searchCoverage.searchFamilies) || !session.searchCoverage.searchFamilies.length) errors.push('searchCoverage.searchFamilies must contain at least one role family');
    if (!Array.isArray(session.searchCoverage.attempts) || !session.searchCoverage.attempts.length) errors.push('searchCoverage.attempts must contain each search attempt');
    ...
    asArray(session.searchCoverage.attempts).forEach((attempt, index) => {
      ...
      if (!statuses.has(attempt?.status)) errors.push(`searchCoverage.attempts[${index}].status is invalid`);
      if (!strategies.has(attempt?.strategy)) errors.push(`searchCoverage.attempts[${index}].strategy is invalid`);
      if (attempt?.strategy === 'employer-expansion') requiredString(attempt?.employer, `searchCoverage.attempts[${index}].employer`, errors);
    });
  }
  ...
  asArray(session.leads).forEach((lead, index) => {
    for (const field of ['title', 'employer', 'source', 'url', 'roleFamily', 'discoveredAt']) requiredString(lead?.[field], `leads[${index}].${field}`, errors);
    if (!leadStatuses.has(lead?.status)) errors.push(`leads[${index}].status is invalid`);
    if (typeof lead?.employerExpansionRequired !== 'boolean') errors.push(`leads[${index}].employerExpansionRequired must be boolean`);
    requiredString(lead?.employerExpansionReason, `leads[${index}].employerExpansionReason`, errors);
    if (lead?.status !== 'assessed' && !asText(lead?.reason)) errors.push(`leads[${index}].reason is required when status is ${lead?.status}`);
  });
  ...
  if (session.candidate) {
    ...
    if (!Array.isArray(session.candidate.availabilityWindows)) errors.push('candidate.availabilityWindows must be an array');
    asArray(session.candidate.availabilityWindows).forEach((window, index) => {
      ...
      if (!(Number(window?.maxHoursPerWeek) > 0)) errors.push(`candidate.availabilityWindows[${index}].maxHoursPerWeek must be positive`);
      for (const field of ['startAt', 'endAt']) {
        if (window?.[field]) {
          try { parseDate(window[field], `candidate.availabilityWindows[${index}].${field}`); }
          catch (error) { errors.push(error.message); }
        }
      }
    });
    if (session.candidate.workRights) {
      if (!['temporary', 'unrestricted', 'none'].includes(session.candidate.workRights.status)) errors.push('candidate.workRights.status is invalid');
      if (typeof session.candidate.workRights.unrestricted !== 'boolean') errors.push('candidate.workRights.unrestricted must be boolean');
    }
    for (const [field, values] of [['skills', session.candidate.skills], ['capabilities', session.candidate.capabilities]]) {
      asArray(values).forEach((item, index) => {
        ...
        if (!Object.hasOwn(LEVEL_WEIGHT, item?.level)) errors.push(`candidate.${field}[${index}].level is invalid`);
      });
    }
  }

  asArray(session.jobs).forEach((job, index) => {
    for (const field of ['source', 'title', 'employer', 'sourceUrl', 'location', 'programmeType', 'contractType', 'workload']) requiredString(job?.[field], `jobs[${index}].${field}`, errors);
    requiredString(job?.verificationEvidence?.verifiedAt, `jobs[${index}].verificationEvidence.verifiedAt`, errors);
    if (!job?.dateEvidence || typeof job.dateEvidence !== 'object' || Array.isArray(job.dateEvidence)) errors.push(`jobs[${index}].dateEvidence must be an object`);
    for (const field of ['detailPageOpened', 'applyRouteAvailable', 'expiredIndicatorVisible', 'unavailableIndicatorVisible']) {
      if (typeof job?.verificationEvidence?.[field] !== 'boolean') errors.push(`jobs[${index}].verificationEvidence.${field} must be boolean`);
    }
    if (!['internship', 'graduate', 'standard', 'not-stated'].includes(job?.programmeType)) errors.push(`jobs[${index}].programmeType is invalid`);
    if (!['fixed-term', 'permanent', 'casual', 'contract', 'not-stated'].includes(job?.contractType)) errors.push(`jobs[${index}].contractType is invalid`);
  ...
  asArray(session.relatedOpportunities).forEach((item, index) => {
    for (const field of ['kind', 'title', 'organisation', 'url', 'registrationStatus']) requiredString(item?.[field], `relatedOpportunities[${index}].${field}`, errors);
    requiredString(item?.verificationEvidence?.verifiedAt, `relatedOpportunities[${index}].verificationEvidence.verifiedAt`, errors);
    for (const field of ['detailPageOpened', 'applyRouteAvailable', 'expiredIndicatorVisible', 'unavailableIndicatorVisible']) {
      if (typeof item?.verificationEvidence?.[field] !== 'boolean') errors.push(`relatedOpportunities[${index}].verificationEvidence.${field} must be boolean`);
    }
    if (!['event', 'programme', 'talent-pool', 'recruitment-channel'].includes(item?.kind)) errors.push(`relatedOpportunities[${index}].kind is invalid`);
    if (!['open', 'closed', 'conditional', 'unknown'].includes(item?.registrationStatus)) errors.push(`relatedOpportunities[${index}].registrationStatus is invalid`);
  });
...
function semanticSimilarity(left, right) {
  ...
  if (group >= 0 && group === conceptGroup(b)) return 0.82;
...
function bestSemanticMatch(values, targets) {
  let best = 0;
  for (const value of asArray(values)) for (const target of asArray(targets)) best = Math.max(best, semanticSimilarity(value, target));
  return best;
}
...
function evaluateConstraint(actual, constraints, label, blockers, cautions, positives) {
  ...
  if (!actual) { cautions.push(`${label} is not stated`); return 0.25; }
  if (constraints.some((item) => valueMatches(actual, item.value))) { positives.push(`${label}: ${actual}`); return 1; }
  const hard = constraints.filter((item) => item.strength === 'hard');
  if (hard.length) blockers.push(`${label} ${actual} is outside the hard preference: ${hard.map((item) => item.value).join(', ')}`);
  else cautions.push(`${label} ${actual} differs from the soft preference: ${constraints.map((item) => item.value).join(', ')}`);
  return 0;
}
...
function evaluateAvailability(job, candidate, dates) {
  ...
  if (!windows.length) return { status: 'unknown', reason: 'No candidate availability windows were recorded' };
  if (!dates.startAt.value || !dates.endAt.value) return { status: 'unknown', reason: 'Job start/end dates were not both verified' };
...
function evaluateWorkRights(job, candidate, dates) {
  ...
  if (!requirement) return { status: 'unknown', reason: 'No explicit work-right requirement was recorded' };
  if (!rights || typeof rights !== 'object') return { status: 'unknown', reason: 'Candidate work-right details are not structured' };
  if (requirement.country && rights.country && !valueMatches(requirement.country, rights.country)) return { status: 'incompatible', reason: `Work rights are for ${rights.country}, not ${requirement.country}` };
  if (requirement.requiresUnrestricted === true && rights.unrestricted !== true) return { status: 'incompatible', reason: 'The role requires unrestricted work rights' };
  if (requirement.requiresCurrentRights === true && rights.status === 'none') return { status: 'incompatible', reason: 'The role requires current local work rights' };
  if (rights.validUntil && dates.endAt.value && parseDate(rights.validUntil, 'workRights.validUntil') < parseDate(dates.endAt.value, 'endAt')) return { status: 'incompatible', reason: 'Current work rights expire before the job ends' };
  if (['temporary', 'unrestricted'].includes(rights.status)) return { status: 'compatible', reason: 'Recorded work rights satisfy the stated requirement for the verified job period' };
...
export function scorePracticalFit(candidate, preferences, job) {
  ...
  const cautions = [];
  let score = 0;
  if (job.engagementModel && normalise(job.engagementModel) !== 'employee') blockers.push(`Engagement model is ${job.engagementModel}, not employee employment`);
  ...
  if (remote) { positives.push('Fully remote'); score += 1.5; }
  else score += evaluateConstraint(job.location, legacyConstraints(preferences, 'location', preferences.locations ?? candidate.locations), 'Location', blockers, cautions, positives) * 1.5;
...
export function classifyVerification(job, preferences, now = new Date()) {
  ...
  let hostname = '';
  try { hostname = new URL(job.sourceUrl).hostname.toLowerCase(); }
  catch { status = 'rejected'; reasons.push('Source URL is invalid'); }
  if (BLOCKED_AGGREGATORS.some((domain) => hostname.includes(domain))) { status = 'rejected'; reasons.push('Final link is an aggregator rather than a permitted direct vacancy page'); }
  const evidence = job.verificationEvidence ?? {};
  if (evidence.expiredIndicatorVisible) { status = 'closed'; reasons.push('The page visibly says the vacancy is expired or closed'); }
  if (evidence.unavailableIndicatorVisible) { status = 'unavailable'; reasons.push('The page visibly says the vacancy is removed or unavailable'); }
  if (!evidence.detailPageOpened && status === 'verified-active') { status = 'unverified'; reasons.push('The exact job detail page was not directly opened'); }
  if (!evidence.applyRouteAvailable && status === 'verified-active') { status = 'unverified'; reasons.push('No working application route or current application instructions were verified'); }
  const dates = resolvedJobDates(job);
  ...
  if (dates.closesAt.value && closingHasPassed(dates.closesAt.value, now)) { status = 'closed'; reasons.push(`Closing date ${dates.closesAt.value} has passed`); }
  const maxAge = Number(preferences.maxPostingAgeDays ?? 30);
  if (!dates.postedAt.value) {
    if (status === 'verified-active') status = 'unverified';
    reasons.push('Posting date is unavailable or conflicting');
  } else {
    const age = postingAgeDays(dates.postedAt.value, now);
    if (age > maxAge) { status = 'rejected'; reasons.push(`Posted ${age} days ago, outside the ${maxAge}-day window`); }
    if (age < -1 && status === 'verified-active') { status = 'unverified'; reasons.push('Posting date is in the future'); }
  }
...
function classifyRelatedOpportunity(item) {
  ...
  if (evidence.expiredIndicatorVisible || evidence.unavailableIndicatorVisible || item.registrationStatus === 'closed') return 'closed';
  if (!evidence.detailPageOpened) return 'unverified';
  if (item.registrationStatus === 'open') return 'verified-open';
  if (item.registrationStatus === 'conditional') return 'conditional';
  return 'unverified';
}
...
function sourceCoverage(attempts) {
  ...
  for (const attempt of attempts) {
    ...
    if (attempt.note) previous.notes.push(attempt.note);
...
export function renderMarkdown(report) {
  ...
  if (coverage.status !== 'complete') lines.push('> Search coverage was incomplete. Results describe only the public sources accessible in this run; additional suitable vacancies may exist.', '');
  if (!report.recommended.length) lines.push(coverage.status === 'complete' ? 'Today there are no new qualified vacancies.' : 'No qualified roles were verified among accessible sources; this is not evidence that no suitable vacancies exist.', '');
  else {
  ...
  if (!report.manualVerification.length) lines.push('- None', '');
  else {
  ...
  if (!report.relatedOpportunities.length) lines.push('- None', '');
  else {
    ...
    for (const item of report.relatedOpportunities) lines.push(`- **${item.title} — ${item.organisation}** (${item.kind}; ${item.status}): ${item.audience ?? 'Audience not stated'}${item.conditions ? `; ${item.conditions}` : ''}. [Official information](${item.url})`);
    lines.push('');
  }
  ...
  for (const job of [...report.recommended, ...report.stretch, ...report.manualVerification, ...report.closed, ...report.incompatible, ...report.lowFit, ...report.otherUnverified]) lines.push(jobMarker(job));
  for (const item of report.relatedOpportunities) lines.push(opportunityMarker(item));
  return `${lines.join('\n')}\n`;
}
...
export function extractReportHistory(markdown) {
  ...
  if (!states.size) {
    for (const match of markdown.matchAll(/\((https?:\/\/[^)\s]+)\)/g)) legacyIdentities.add(`url:${normaliseUrl(match[1])}`);
...
async function loadReportHistory(outputPath, now, maxPostingAgeDays) {
  ...
  for (const name of names) {
    ...
    if (match && match[1] >= earliest && match[1] <= today) paths.add(join(folder, name));
  }
  const states = new Map();
  const legacyIdentities = new Set();
  let currentMarkdown;
...
function filterHistoricalItems(items, history, identityFunction, fingerprintFunction) {
  ...
  for (const original of items) {
    ...
    if (sameState || legacyMatch) { excluded += 1; continue; }
    if (identities.some((identity) => history.states.has(identity))) { item.historyChange = 'updated'; updated += 1; }
    fresh.push(item);
  }
...
export async function writeReport(inputPath, outputPath, options = {}) {
  ...
  if (history.currentMarkdown !== undefined) {
    if (!newItems) { report.writeAction = 'unchanged'; return report; }
...
export async function runCli(argv = process.argv.slice(2)) {
  try {
    ...
    if (args.help || !args.command) { console.log(usage()); return 0; }
    ...
    if (args.command === 'report') {
      ...
      if (report.writeAction === 'unchanged') console.log(`No new or changed items; existing report left unchanged: ${resolve(args.output)} (${report.excludedPreviouslyReported} unchanged item(s))`);
      else {
